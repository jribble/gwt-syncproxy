...
public class ProfileServiceTest extends TestCase{
  ...
  public ProfileServiceTest()  {
    try {
      userSession = LoginUtils.loginAppEngine("http://localhost:8888", "http://localhost:8888", EMAIL, "");
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void testUserProfile() throws UnauthenticateException {
    service = 
      (ProfileService)SyncProxy.newProxyInstance(
          ProfileService.class, RPCSyncTestSuite.BASE_URL, 
          "profile", userSession);
  ...
  public void testUserProfile2() {
    ...
    UserInfo userInfo;
    try {
      userInfo = guestSessionService.getMyProfile();
      fail();
